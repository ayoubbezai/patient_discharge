...
import { Calendar, MapPin, Users, Trophy, Plus, Eye, Loader2 } from 'lucide-react';
        ...
        <div className="px-2 py-2 bg-gradient-to-br from-slate-50 to-slate-100 min-h-screen">
          ...
          <Card className="max-w-md mx-auto mt-8 border-red-200 bg-red-50">
            <CardContent className="pt-4 text-center">
              <p className="text-red-700 font-medium text-sm">Failed to load matches</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
            <Card className="border-slate-200 shadow-sm">
              <CardContent className="p-2">
                <p className="text-xs text-slate-600">Upcoming Matches</p>
            ...
            <Card className="border-slate-200 shadow-sm">
              <CardContent className="p-2">
                ...
                <p className="text-xl font-bold text-slate-900 mt-0.5">{matches.length}</p>
              </CardContent>
            </Card>
          </div>
          ...
          {filteredMatches.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
              {filteredMatches.map((match: any) => (
                <Card key={match.id} className="border-slate-200 shadow-sm hover:shadow-md transition-shadow">
                  <CardContent className="p-3 space-y-2">
                    <div className="flex items-start justify-between">
                      <div>
                        <p className="font-bold text-sm text-slate-900">Match #{match.id.slice(0, 8)}</p>
